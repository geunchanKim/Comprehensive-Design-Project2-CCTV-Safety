...
첫 구현에서는 설명과 재현이 쉬운 규칙 기반 판정을 사용하고, 실제 영상 검증 결과에 따라 임계값을 조정합니다.

| 위험 유형 | 입력 정보 | 판정 기준 초안 | 오탐 방지 방식 |
| --- | --- | --- | --- |
| 위험구역 진입 | 작업자 바운딩박스, 구역 다각형 | 바운딩박스 하단 중앙점이 구역 내부에 위치 | 일정 프레임 이상 연속 진입 시 이벤트 확정 |
| 작업자–지게차 접근 | 두 객체의 위치, 추적 ID, 이동 방향 | 영상 좌표상 거리 감소와 설정 거리 이하 접근을 함께 확인 | 원근 왜곡을 고려해 카메라별 임계값 설정 |
| 장시간 체류 | 작업자 추적 ID, 구역 진입·이탈 시각 | 지정 구역 내부 체류 시간이 기준을 초과 | 추적 ID 변경 시 짧은 공백 구간을 연결 |
| 반복 알림 | 위험 유형, 객체 ID, 발생 시각 | 같은 객체·구역의 이벤트를 묶어 관리 | 쿨다운 시간 이후 재알림 허용 |

영상 좌표의 거리는 실제 미터 단위 거리와 다릅니다. 실제 거리 기반 판단이 필요하면 카메라 캘리브레이션이나
호모그래피를 적용하고, 적용 전에는 결과를 상대적인 접근 위험 지표로만 사용합니다.

### 위험 이벤트 데이터 예시

AI 분석, 백엔드, 관제 화면이 같은 형식을 기준으로 연동할 수 있도록 최소 이벤트 필드를 정의합니다.

```json
{
  "event_id": "evt-20260917-0001",
  "camera_id": "camera-01",
  "event_type": "restricted_zone_entry",
  "severity": "warning",
  "occurred_at": "2026-09-17T14:30:00+09:00",
  "zone_id": "zone-A",
  "track_ids": [12],
  "confidence": 0.91,
  "snapshot_uri": null,
...
필드명과 위험도 단계는 API 설계 시 확정하며, 시각은 타임존이 포함된 ISO 8601 형식으로 저장합니다.
...
- [ ] **요구사항 정의**: 대상 현장, 작업자·지게차 탐지 범위, 위험구역 진입 기준 정리
- [ ] **데이터셋 선정**: 후보 샘플·이용 조건 비교, 팀 협의 후 데이터셋과 클래스 확정
- [ ] **영상 입력 구성**: 테스트 영상 확보 및 영상 읽기 기능 구현
- [ ] **객체 탐지 구현**: YOLO 모델 선정 및 작업자·지게차 탐지 결과 확인
...
CCTV 영상에는 개인을 식별할 수 있는 정보가 포함될 수 있으므로 개발·시연 단계에서도 필요한 데이터만 사용합니다.

- 촬영 또는 제공받은 영상의 사용 목적과 이용 권한을 확인하고, 허가되지 않은 원본은 저장소에 커밋하지 않습니다.
- 원본 영상, 모델 가중치, 이벤트 클립과 비밀 설정값은 Git에서 제외하고 접근 권한이 제한된 저장소에 보관합니다.
- 이벤트에는 업무에 필요한 최소 정보만 저장하고, 보존 기간이 끝난 영상과 스냅샷은 삭제합니다.
- 화면 공유와 시연 시 얼굴·차량번호 등 불필요한 식별 정보는 마스킹하는 방안을 검토합니다.
- 본 시스템의 탐지 결과는 관리자의 판단을 지원하는 정보이며, 단독으로 안전을 보장하거나 자동 제재의 근거로 사용하지 않습니다.
...
- **데이터 관리**: 영상 수집 동의·접근 권한·보존 기간·비식별화 및 삭제 절차
