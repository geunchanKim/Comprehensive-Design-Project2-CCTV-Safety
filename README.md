...
프로젝트는 **구현 및 검증 진행 중**입니다. Unity 가상환경과 백엔드 기반 작업이 병행되고 있으며, YOLO 모델·최종 데이터셋·3D 좌표 복원·위험 판단은 아직 완성된 기능이 아닙니다. 백엔드의 FastAPI·PostgreSQL·Docker 구성은 별도 PR에서 검토 중이므로 이 README에서는 실행 가능한 통합 시스템으로 소개하지 않습니다.
...
현재 브랜치에는 기획 문서인 README만 있습니다. Unity·백엔드 등은 별도 브랜치/PR에서 작업 중이며, 통합 후 설치·실행 방법과 실제 디렉터리 구조를 이 문서에 추가할 예정입니다.
